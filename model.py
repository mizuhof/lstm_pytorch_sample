...
class Model(nn.Module):
    ...
    def forward(self, inputs, future=0):
        ...
        h_t = torch.zeros(inputs.size(0),  512, dtype=torch.float).to(device)
        c_t = torch.zeros(inputs.size(0),  512, dtype=torch.float).to(device)
        h_t2 = torch.zeros(inputs.size(0), 512, dtype=torch.float).to(device)
        c_t2 = torch.zeros(inputs.size(0), 512, dtype=torch.float).to(device)
        ...
        for i, input_t in enumerate(inputs):
            ...
            h_t, c_t = self.lstm1(lstm_input_t, (h_t, c_t))
